...
func TestStop(t *testing.T) {
	...
	// Pod 1: Prepared but no activity yet (waitingForActivity)
	np.allocatedPods[podUID1] = time.Time{}

	// Pod 2: Recent activity (waitingForGrace)
	np.allocatedPods[podUID2] = fakeClock.Now()
	...
	// 2. Advance clock to satisfy grace period for Pod 2.
	// But Pod 1 still has no activity, so Stop should still wait.
	...
	// 3. Simulating activity for Pod 1 (e.g. CreateContainer called).
	np.allocatedPodsMu.Lock()
	np.allocatedPods[podUID1] = fakeClock.Now()
	np.allocatedPodsMu.Unlock()

	// Advance clock again to satisfy grace period for Pod 1.
	fakeClock.Step(15 * time.Second)
