...
func addIPVlan(ifName string, parentLink netlink.Link, containerNs netns.NsHandle, config *apis.SubInterfaceConfig) (*resourceapi.NetworkDeviceData, error) {
	...
	if config != nil && config.IPVlanConfig != nil {
		switch config.IPVlanConfig.Mode {
		case "", "l2":
		...
		default:
			return nil, fmt.Errorf("unsupported ipvlan mode: %s", config.IPVlanConfig.Mode)
		}

		switch config.IPVlanConfig.Flag {
		case "", "bridge":
		...
		default:
			return nil, fmt.Errorf("unsupported ipvlan flag: %s", config.IPVlanConfig.Flag)
		}
	} else {
		mode = netlink.IPVLAN_MODE_L2
		flag = netlink.IPVLAN_FLAG_BRIDGE
