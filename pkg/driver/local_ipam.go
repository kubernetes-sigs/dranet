...
// Allocate finds and returns a free IP address from the given CIDR range,
// excluding reserved addresses and avoiding conflicts with currently allocated IPs on the node.
...
func calculateBounds(cidr netip.Prefix) (bounds, error) {
	...
	if hostsBits >= 64 {
		size = math.MaxInt64 // prevent uint64 overflow
	} else {
		size = uint64(1) << uint(hostsBits)
	}
	size-- // exclude base network address
