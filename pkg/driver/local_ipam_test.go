...
func TestAllocate(t *testing.T) {
		...
		err := store.SetDeviceConfig("pod-occupied", "eth0", DeviceConfig{
			NetworkInterfaceConfigInPod: apis.NetworkConfig{
				Interface: apis.InterfaceConfig{
					Addresses: []string{"2001:db8::3/126"},
				},
			},
		})
		if err != nil {
	...
	t.Run("out of IPs error (IPv4)", func(t *testing.T) {
		...
		err = store.SetDeviceConfig("pod-1", "eth0", DeviceConfig{
			NetworkInterfaceConfigInPod: apis.NetworkConfig{
				Interface: apis.InterfaceConfig{
					Addresses: []string{"192.168.1.3/30"},
				},
			},
		})
		if err != nil {
...
func TestCalculateBounds(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "valid IPv6 /64 CIDR, reserved=6",
			cidr:      netip.MustParsePrefix("2001:db8::/64"),
			wantErr:   false,
			wantFirst: "2001:db8::7",
			wantLast:  "2001:db8::ffff:ffff:ffff:ffff",
		},
		{
			name:      "valid IPv6 /122 CIDR, reserved=2",
			cidr:      netip.MustParsePrefix("2001:db8::/122"),
			wantErr:   false,
			wantFirst: "2001:db8::3",
			wantLast:  "2001:db8::3f",
		},
		{
			name:      "valid IPv4 /30 CIDR",
			cidr:      netip.MustParsePrefix("192.168.1.0/30"),
			wantErr:   false,
			wantFirst: "192.168.1.3",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.wantErr {
				...
				if got.ipFirst.String() != tt.wantFirst {
					t.Errorf("calculateBounds() ipFirst = %s, want %s", got.ipFirst, tt.wantFirst)
				}
				if got.ipLast.String() != tt.wantLast {
					t.Errorf("calculateBounds() ipLast = %s, want %s", got.ipLast, tt.wantLast)
				}
			}
		})
	}
